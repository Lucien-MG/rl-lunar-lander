...
class Agent:
    ...
    def learn(self):
        ...
        # Double q learning using primary network to predict action to take
        next_state_actions = self.primary(next_states).argmax(1).detach()

        # Calculate target reward and detach it from the graph 
        # Avoid gradient descend in the target network
        # target network will only predict value
        next_state_value = self.target(next_states).gather(0, next_state_actions.unsqueeze(-1)).squeeze(-1).detach()
